...
原理:全部题填 'A' 提交,根据返回的 `isRight` 字段把错的题换下一个字母再提交,
直到全部 isRight=True。仅适用于单选,且容易触发服务端风控。

v2 改为直接调 /course/api/v3/answer/.../default 拿加密答案 + 客户端 AES 解密
(见 core/unipus/api_client.py),本文件保留仅作历史参考,不再被主流程引用。
"""
import re
import uuid
